...
bool Socket::sockSend(SOCKET& remoteSocket, const std::string& data) {
    bool returnValue = true;
    try {
        int bytesSent;
        if ((bytesSent = send(remoteSocket, data.data(), static_cast<int>(data.size()), 0)) == SOCKET_ERROR) {
...
bool Socket::sockSend(SOCKET &remoteSocket, const std::vector<uint8_t>& data) {
    bool returnValue = true;
    try {
        int bytesSent;
        if ((bytesSent = send(remoteSocket, reinterpret_cast<const char*>(data.data()), static_cast<int>(data.size()), 0)) == SOCKET_ERROR) {
            throw runtime_error("Send failed with error: " + to_string(WSAGetLastError()));
        }
    }
    catch (std::exception& ex) {
...
bool Socket::sockReceive(SOCKET& remoteSocket, std::vector<uint8_t> &rcvdMessage, string& errorMessage) {
    bool returnValue = true;
    try {
        rcvdMessage.clear();
        int rslt = 0;
        int bufferLen = 4096;
        std::vector<uint8_t> recvBuffer(bufferLen);
        do {
            if ((rslt = recv(remoteSocket, reinterpret_cast<char*>(recvBuffer.data()), bufferLen, 0)) > 0) {
                rcvdMessage.insert(rcvdMessage.end(), recvBuffer.begin(), recvBuffer.begin() + rslt);
                if(rslt == 48) { // complete message received
                    unsigned char firstByte = static_cast<unsigned char>(recvBuffer[0]);
                    unsigned char mode = firstByte & 0x07;
                    if (mode == 4) { // check for NTP server response
                        break;
                    }
                }
            }
            else if (rslt == 0) { // client connection closing
                throw runtime_error("Socket::sockReceive(): Exception: Server closed connection: " + to_string(WSAGetLastError()));
            }
            else {
                throw runtime_error("Socket::sockReceive(): Exception: Server data receive error: " + to_string(WSAGetLastError()));
            }
        } while (rslt > 0);
    }
    catch (std::exception& ex) {
