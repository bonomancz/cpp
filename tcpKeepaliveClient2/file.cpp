...
std::vector<std::string> File::read(const std::string &fileName) {
	try {
...
void File::write(const std::string& fileName, const std::string& data, bool append) {
	std::lock_guard<std::mutex> lock(fileMutex);
	try {
		std::ofstream fout(fileName, append ? std::ios::app : std::ios::trunc);
		if (!fout) {
			throw std::runtime_error("File::write(): Exception: open ofstream failed. (" + fileName + ")");
		}
		fout << data << std::endl;
		if (!fout.good()) {
